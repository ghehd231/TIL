...
//     while(priorities.length >0){//que가 모두 빌때까지 반복
//         const j = priorities.shift();
//         if(priorities.some((e,i)=> e > j)){
//             priorities.push(j);
//         }else{
//             answer = answer+1;
//             if(targetIndex === 0){
...
function solution(priorities, location) {
  ...
  while (arr.length > 0) {
    const j = arr.shift();
    const check = arr.some(e => e.priority > j.priority);
    if (check) {
      arr.push(j);
    } else {
      que.push(j);
  ...
  answer = que.findIndex(e => e.index === location) + 1;
  return answer;
}
